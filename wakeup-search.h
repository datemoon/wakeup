...
{
public:
	WakeupSearch(int sil_state =0,bool adjust=false):adjust_(adjust),cur_frame_(0),max_frame_len_(0), dtw_(NULL), sil_state_(sil_state) {}
	~WakeupSearch()
	{
		delete dtw_;
	}
	...
	// get state align res
	Align_res* GetAliState(int &dtw_len);
	...
	void ResetAliState()
	{
		for(size_t i=0;i<align_states_.size();++i)
			align_states_[i].Reset();
	}
private:
