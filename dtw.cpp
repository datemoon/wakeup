...
int DtwAlign::Init(int *state,int cols)
{
//try{
    if(cols != dtw_cols_){
        return -1;
    }
    state_ = new int[dtw_cols_];
    memset(state_,0x00,dtw_cols_*sizeof(int));
    int i=0;
    for(i=0;i<dtw_cols_;++i)
        state_[i]=state[i];
...
Align_res* DtwAlign::startAlign(int *res_len)
{
    ...
    //look back upon
    {
        ...
        *res_len = end-start;
        ...
        align_[stop_row].state = state_[j];
